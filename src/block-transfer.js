...
export class BlockTransfer {
  ...
  addLane(conn,index,epoch=this.epoch){
    ...
    const lost=()=>{
      ...
      if(this.blocks?.size&&!this.terminal())this.conn?.close();
    };
  ...
  pickLane(){
    ...
    for(const [index,conn] of this.lanes){
      if(!conn?.open)continue;const plan=this.planFor(conn),dc=conn.dataChannel;
  ...
  async receiveControl(m,epoch) {
    if(m.type==='hello') {
      ...
      if(saved) {
        ...
        for(let f=0;f<saved.files.length;f++)if(!saved.files[f].complete){
    ...
    if(m.type==='file-finish') {
      if(!Number.isInteger(m.file)||m.file<0)throw Error('Invalid file index.');const state=this.record.files[m.file],file=this.manifest[m.file];if(!file||!state||state.next!==Math.ceil(file.size/BLOCK_SIZE)||!HEX.test(m.hash)||this.blocks.size)throw Error('Cannot verify an incomplete file.');
  ...
  fail(detail,state='failed',notify=true) {
    ...
    this.epoch++;this.blocks.clear();this.completedData.clear();for(const hash of this.receiveHashes.values())hash.close();this.receiveHashes.clear();this.rejectWaiters(new Interrupted('Transfer stopped'));clearInterval(this.heartbeat);
