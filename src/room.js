...
export class Room {
  ...
  incoming(conn) {
    ...
    if (['file-v2','file-v3','file-v5'].includes(conn.metadata?.kind) && member) this.cb.onTransfer?.(conn,member);
  ...
  connect(id,transferId) {
    ...
    return this.peer.connect(id,{reliable:true,serialization:'raw',metadata:{kind:transferId?'file-v5':'file-v2',transferId,lane:0}});
  }
