...
export function mergePresenceRecords(local,remote,now=Date.now()){
 ...
 for(const source of [...local,...remote]){
  if(!validPresence(source,now))continue;
  const row=cleanPresence(source);
  if(!merged.has(row.uuid)||comparePresence(merged.get(row.uuid),row)<0)merged.set(row.uuid,row);
 }
...
export class PresenceDB{
 constructor(indexed=indexedDB,name='wft-presence-v1'){this.indexed=indexed;this.name=name;this.database=null;}
 ...
 async merge(incoming,now=Date.now()){
  ...
  return new Promise((resolve,reject)=>{const tx=db.transaction('presenceRecords','readwrite'),store=tx.objectStore('presenceRecords'),request=store.getAll();let merged;
   request.onsuccess=()=>{try{
    ...
    merged=mergePresenceRecords(existing,incoming,now);
    const keep=new Set(merged.map(row=>row.uuid));
    for(const row of existing)if(!keep.has(row.uuid))store.delete(row.uuid);
