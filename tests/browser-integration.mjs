...
try{
  ...
  for(const page of [a,b]){page.on('pageerror',e=>console.error('PAGE ERROR',e.message));await page.goto(`http://127.0.0.1:${server.address().port}`);await page.waitForFunction(()=>window.testApi);await page.evaluate(relay=>{window.peerSettings={peerOptions:testApi.peerOptions()};if(relay)peerSettings.peerOptions.config.iceTransportPolicy='relay';},process.env.RELAY_ONLY==='1');}
  const code=await a.evaluate(async()=>{const {Room,newCode}=testApi;window.errors=[];window.members=[];window.room=new Room({onMembers:m=>window.members=m,onError:e=>window.errors.push(e)},peerSettings);const code=newCode();await room.open(code,true,'Sender','send');return code;});
  await b.evaluate(async code=>{const {Room,Transfer,memorySink}=testApi;window.members=[];window.received=[];window.errors=[];window.room=new Room({onMembers:m=>window.members=m,onError:e=>window.errors.push(e),onTransfer:conn=>{window.transfer=new Transfer(conn,{onOffer:()=>window.offered=true,onFile:async f=>window.received.push({name:f.name,bytes:Array.from(new Uint8Array(await f.blob.arrayBuffer()))})});}},peerSettings);await room.open(code,false,'Receiver','receive');},code);
